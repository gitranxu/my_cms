...
//在生成文件之前，判断当前文件名是否已被占用
router.get('/is_file_url_exist', function(req, res, next) {
	var _pool = sqlclient.init()._pool;
	var url = _pool.escape(req.query.url);
	var col = req.query.col;
	var is_file_url_exist_sql = "SELECT 1 FROM c_generate_html_config c WHERE c.`"+col+"`="+url;
	console.log(is_file_url_exist_sql+'--------------------------------is_file_url_exist_sql');
	sqlclient.query(is_file_url_exist_sql,function(err,rows,fields){
		if(err) throw err;
		res.json({reCode:1,msg:rows.length});
	});
});
...
router.post('/generate_html', function(req, res, next) {
	...
	var lid = _pool.escape(req.body.lid);
...
router.post('/generate_edit_html', function(req, res, next) {
	...
	var html = '<!DOCTYPE html>'+
				...
				'<body>'+
					...
					'<script src="/javascripts/other/jquery.ztree.all.min.js" class="need_remove"></script>'+
					...
					'<script src="/javascripts/uuid.js" class="need_remove"></script>'+
